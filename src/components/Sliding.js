        ...
        {slides.map((slide, index) => {
          return (
            <HeroSlide key={index}>
              {index === current && (
                <HeroSlider>
                  <HeroImage src={slide.images[current % slide.images.length].url} alt={slide.images[current % slide.images.length].name} />
                  <HeroContent>
                    <h1>{slide.name}</h1>
                    <p>{slide.listingType}: Ksh {slide.priceFrom}</p>
                    <p>Location: {slide.town}</p>
